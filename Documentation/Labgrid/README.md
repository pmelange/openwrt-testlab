...
* Modify ```labgrid/config/systemd/labgrid-exporter.servie``` so that the ExecStart looks like this:

```
ExecStart=/opt/labgrid-venv/bin/labgrid-exporter -i /etc/labgrid/exporter.yaml
```
