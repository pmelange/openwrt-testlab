...
@attr.s(eq=False)
class BBBConfigStrategy(Strategy):
    """BBBConfigStrategy - Strategy to boot from flash"""
    bindings = {
        "power": "PowerProtocol",
    ...
    def transition(self, status):
        ...
        match status:
            ...
            case Status.config:
                ...
                result, _, _ = self.uci.get("system",
                                            "@system[0]",
                                            "compat_version")
                if "9.9" == result[0]:
                    # This is a preconfigured bbb-configs setup.  Do nothing
                    pass
                else:
                    # This is not a bbb-configs setup.  Probably a ramboot
                    # so configuration is needed to be able to flash
