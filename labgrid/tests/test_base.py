...
def test_default_route_exists(configured, shell_command):
    _, _, errorcode = shell_command.run("ip r g 8.8.8.8")
    assert errorcode == 0

@pytest.mark.lg_feature("online")
def test_ping_default_route(configured, shell_command):
    result, _, _ = shell_command.run("ip route | grep -m 1 ^default | cut -d ' ' -f 3")
    _, _, errorcode = shell_command.run(f"ping -c 3 {result[0]}")
    assert errorcode == 0

@pytest.mark.lg_feature("online")
def test_ping_dot8(configured, shell_command):
